...
func normalizeUserOrder(order string) string {
	switch strings.ToLower(strings.TrimSpace(order)) {
	case "id_asc", "asc", "id asc":
		return "id asc"
	default:
		return "id desc"
	}
...
func GetAllUsers(pageInfo *common.PageInfo, order string) (users []*User, total int64, err error) {
	// Start transaction
	tx := DB.Begin()
	if tx.Error != nil {
...
func SearchUsers(keyword string, group string, startIdx int, num int, order string) ([]*User, int64, error) {
	var users []*User
	var total int64
	var err error
...
func GetUserQuota(id int, fromDB bool) (quota int, err error) {
	// Do not asynchronously write an unfenced quota-only snapshot: a credit or
	// the existing atomic wallet reservation may have committed since this read.
	// Full authentication cache fills already use the user generation fence.
