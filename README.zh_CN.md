...
| `RELAY_NON_STREAM_TIMEOUT_SECONDS` | 非流式 Relay 总预算，覆盖请求体上传和响应体读取；与调用方 deadline 同时存在时较早者生效；`0` 表示禁用 | `1200` |
| `RELAY_OUTER_PROXY_TIMEOUT_SECONDS` | 用于启动时校验超时层级的已知外层反向代理预算；必须与实际部署的 Caddy/OpenResty 配置一致 | `1260` |
