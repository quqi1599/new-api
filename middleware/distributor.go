...
func Distribute() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if err != nil {
			if common.IsBodyAdmissionError(err) {
				abortWithOpenAiMessage(
					...
					"request body capacity is temporarily exhausted",
					types.ErrorCodeRequestBodyCapacity,
				)
				return
			}
		...
		tokenPolicyBanned, err := model.IsTokenProtectedChannelBanned(c.GetInt("token_id"))
		if err != nil {
			common.SysLog("failed to load API key policy protection state: " + err.Error())
		...
		if tokenPolicyBanned {
			excludedChannelIds, err = model.GetAPIKeyPolicyProtectedChannelIds()
			if err != nil {
				common.SysLog("failed to load API key policy protected channels: " + err.Error())
				abortWithOpenAiMessage(c, http.StatusInternalServerError, common.TranslateMessage(c, i18n.MsgDatabaseError))
				return
			}
		}
...
func getModelFromRequest(c *gin.Context) (*ModelRequest, error) {
	...
	if err != nil {
		// Preserve typed request-body failures for the outer distributor to map
		// admission exhaustion to a stable 503 instead of flattening it into 400.
		return nil, err
	}
...
func getModelRequest(c *gin.Context) (*ModelRequest, bool, error) {
	...
	if strings.Contains(c.Request.URL.Path, "/mj/") {
		...
		if relayMode == relayconstant.RelayModeMidjourneyTaskFetch ||
			...
			if err != nil {
				return nil, false, fmt.Errorf("%s: %w", i18n.T(c, i18n.MsgDistributorInvalidMidjourney, map[string]any{"Error": err.Error()}), err)
			}
